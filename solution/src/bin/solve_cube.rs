...
fn main() {
    ...
    let avg = 1.75;
    // [di, rj, -di, -rj] や [di, di, rj, -di, -di, -rj] という操作が3-rot×2となり有用なので対応する巡回置換を前計算しておく
    ...
    for iter in 0.. {
        ...
        use_exact_cost |= input.color_type == ColorType::Colorful || crt_diff < 3.0 * input.n2 as f64;
        let (can_use_list, crt_cost) = if use_exact_cost {
            decompose_clusters(&input)
        } else {
            (vec![], 0)
        };
        eprintln!(
        ...
        if use_exact_cost {
            ...
            let ret = (0..=out.len())
                    ...
                    for c in 0..can_use_list.len() {
                        ...
                        for &(i, j, k, w) in &can_use_list[c] {
                            ...
                            for mv in input.rot3.get(cluster, i2, k2, j2) {
                                ...
                                let key = (
                                    (mv.len() as i32 - ov) as f64,
                                    (mv.len() as i32 - ov) as f64 / (-w).max(0.01),
                                    0,
                                    -1.0,
                                );
                                if best.can_insert(key) {
                                    best.insert(key, (t, c, w, mv));
                                }
                            }
                        }
                    }
                ...
                .collect::<Vec<_>>();
            let mut best = ret.into_iter().map(|r| r.list()).flatten().collect_vec();
            ...
            if best.len() > 0 {
                ...
                min = best[0].0;
                best_t = best[0].1 .0;
                best_mv = best[0].1 .3.clone();
            }
            best.retain(|b| b.0 .0 <= min.0 * 2.0);
            dbg!(best.len());
            eprintln!("min = {:?}", min);
            let best2 = best
                .into_par_iter()
                .map(|(_, (t, c0, w0, mv))| {
                    ...
                    let t0 = (0..out.len().min(out2.len()))
                        .find(|&t| out[t] != out2[t])
                        .unwrap_or(out.len().min(out2.len()));
                    let t1 = out2.len()
                        - (0..out.len().min(out2.len()))
                            .find(|&t| out[out.len() - 1 - t] != out2[out2.len() - 1 - t])
                            .unwrap_or(out.len().min(out2.len()));
                    let mut cids = cidss[t0].clone();
                    let mut best = (1000.0, 0.0, 0, 0.0);
                    for t in t0..=t1 {
                        let mut ids = mat![!0; input.clusters.len(); 24];
                        for x in 0..6 * input.n2 {
                            if cids[x].0 != !0 {
                        ...
                        for c in 0..can_use_list.len() {
                            ...
                            for &(i, j, k, w) in &can_use_list[c] {
                                ...
                                let k2 = ids[c][k];
                                for mv in input.rot3.get(cluster, i2, k2, j2) {
                                    let ov = overlap(&out2[..t], &mv, n) + overlap(&mv, &out2[t..], n);
                                    let len = (out2.len() as i32 - out.len() as i32 + mv.len() as i32 - ov) as f64;
                                    let key = (len / 2.0, len / (-w0 - w).max(0.01), 1, -2.0);
                                    best.setmin(key);
                                }
                            }
                        }
                        if t < t1 {
                            apply_fast(&input, &mut cids, out2[t]);
                        }
                    }
                    (best, t, mv)
                })
                .collect::<Vec<_>>();
            for (best, t, mv) in best2 {
                if min.setmin(best) {
                    best_t = t;
                    best_mv = mv;
                }
            ...
            best_out = out[..best_t].to_vec();
            extend_with_overlap(&mut best_out, &best_mv, n);
            extend_with_overlap(&mut best_out, &out[best_t..], n);
        }
        ...
        let best = best
            ...
            .map(|((_, move_type), t, mv)| {
                ...
                let diff_cost = if use_exact_cost {
                    (crt_cost - decompose_clusters_cost(&input)) as f64
                } else {
                    d
                };
                (d, diff_cost, move_type, out2)
            })
            .collect::<Vec<_>>();
...
fn decompose_clusters(input: &Input) -> (Vec<Vec<(usize, usize, usize, f64)>>, i32) {
    ...
    let mut sum = 0;
    for (cid, cluster) in input.clusters.iter().enumerate() {
        let mut g = vec![vec![]; 24];
        match cluster {
            Cluster::Edge(vs) => {
                for i in 0..24 {
                    for j in 0..24 {
                        if input.start[vs[i].0] == input.target[vs[j].0] && input.start[vs[i].1] == input.target[vs[j].1] {
                            g[i].push(j);
                        }
                    }
                }
            ...
            Cluster::Inner(vs, _) => {
                for i in 0..24 {
                    for j in 0..24 {
                        if input.start[vs[i]] == input.target[vs[j]] {
                            g[i].push(j);
                        }
                    }
                }
                ...
                for k in 0..24 {
                    if can_use[i][j][k] {
                        ...
                        for a in match cluster {
                            ...
                            for i in 0..3 {
                                if input.start[a[i]] != input.target[a[i]] {
                                    if input.color_type == ColorType::Colorful {
                                        if input.start[a[i]] / input.n2 as u16 + input.target[a[i]] / input.n2 as u16 == 5 {
                                            d -= OPPOSITE_WEIGHT;
                                        } else {
                                            d -= 1.0;
                                        }
                                    } else {
                                        if input.start[a[i]] + input.target[a[i]] == 5 {
                                            d -= OPPOSITE_WEIGHT;
                                        } else {
                                            d -= 1.0;
                                        }
                                ...
                                if input.start[a[i]] != input.target[a[(i + 1) % 3]] {
                                    if input.color_type == ColorType::Colorful {
                                        if input.start[a[i]] / input.n2 as u16 + input.target[a[(i + 1) % 3]] / input.n2 as u16
                                            == 5
                                        {
                                            d += OPPOSITE_WEIGHT;
                                        } else {
                                        ...
                                        if input.start[a[i]] + input.target[a[(i + 1) % 3]] == 5 {
                                            d += OPPOSITE_WEIGHT;
                                        } else {
                                            d += 1.0;
                                        }
...
fn decompose_clusters_cost(input: &Input) -> i32 {
    let mut sum = 0;
    for cluster in input.clusters.iter() {
        ...
        fn decompose_loops_rec(
            ...
            if crt.len() > 0 && crt[0] == u {
                ...
                for s in 0..24 {
                    if used >> s & 1 == 0 {
                        decompose_loops_rec(g, loops, &mut vec![], s, used, crt_cost, false, min);
                        loops.pop();
                        return;
                    }
                }
                ...
                for &v in &g[u] {
                    if used >> v & 1 == 0 {
                        decompose_loops_rec(g, loops, crt, v, used | 1 << v, crt_cost, single_used, min);
                    }
                }
                crt.pop();
            }
        ...
        let mut min = INF;
        decompose_loops_rec(&g, &mut vec![], &mut vec![], 0, 0, 0, false, &mut min);
        sum += min;
    }
    sum
}
