...
fn init_optimize(input: &Globe, start: &mut Vec<u16>, avg: f64) -> Vec<usize> {
    ...
    'lp: for r0 in 0..input.rows / 2 {
        ...
        let mut used = HashSet::new();
        used.insert(get_hash(&start1, &hash, rows, cols, false));
        ...
        for t in 0.. {
                    ...
                    .map(|&(_, t2, b, k)| {
                        ...
                        let (diff, diff2, rot) = {
                            ...
                            if r != !0 {
                                ...
                                (state.diff, state.diff2, state.rotate)
                            }
                        };
                        (t2, b, k, get_hash(&next, &hash, rows, cols, false), diff, diff2, rot, odd)
                    })
                    .collect::<Vec<_>>();
                list
            };
            ...
            for (t2, b, k, hash, diff, diff2, rot, odd) in list {
                if (0..rows / 2).any(|r| count[r][rot[r] as usize] < max_size)
                    && (diff.iter().sum::<u8>() == 0 || used.insert(hash))
                {
                    let mut next = beam[t2 as usize][b as usize].crt.clone();
                    if (k as usize) < cols {
                        input.apply(&mut next, k as usize);
...
fn compute_eval(adj_diff: u8, _diff: u8, _diff2: u8) -> i32 {
    adj_diff as i32
}
            ...
            for j in 0..=outs[r].len() {
                for di in 0..dp[i][j].len() {
                    for dj in 0..cols {
                        ...
                        if rows == 2 {
                            if i < out.len() && out[i] < cols {
                                if dp[i + 1][j][di][dj].0.setmax(crt) {
                                    dp[i + 1][j][di][dj].1 = trace.add((out[i] + di) % cols, id);
                                }
                            }
                        } else {
                            if i < out.len()
